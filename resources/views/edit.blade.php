...
@section('content')
    ...
    <form action="/update" method="POST" role="form">
        ...
        <div style="display: inline-block; width:45%; margin: 0px 0px 16px">
            ...
            <input required type="number" class="form-control" id="rutInput"  placeholder="Ingrese su Rut" name="rutInput" value="{{ $user->Rut }}">
        </div>
        <div style="display: inline-block; float: right; width:45%; margin: 0px 0px 16px">
            <label for="dateInput">Fecha de nacimiento</label>
            <input required type="text" class="form-control" id="dateInput" placeholder="Seleccione su fecha de nacimiento" name="dateInput" value="{{ $user->FechaNacimiento }}">
        </div>
        <div class="form-group">
